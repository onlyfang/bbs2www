...
main ()
{
  ...
  if (!(cookie.level & PERM_SYSOP))
	...
	{
	  sprintf (buf1, "抱歉,你没有在%s讨论区发表文章的权限!", board);
	  show_error (buf1);
	}
  ...
  if (HIDE_IP == 1)
    sprintf (buf1, "\x1b[1;%2dm※ 来源:．%s %s. [FROM: WWW] \x1b[m\n",
	     color, BBSNAME, BBSHOST);
  else
    sprintf (buf1,
	     "\x1b[1;%2dm※ 来源:．%sWWW %s. [FROM: %.20s] \x1b[m\n",
	     color, BBSNAME, BBSHOST,
