...
main ()
{
  ...
  cgi_html_head ();
  printf ("<title>%s讨论区同主题文章列表</title>\n", BBSID);
  printf ("<body>\n");
  printf ("<center>\n");
  ...
  if (total == 0 && (ch == 'N'))
	...
	default:
      ...
      printf ("<tr><th class=body>序号<th class=body>作者");
      printf
	("<th class=body>日期<th class=body>标题<th class=body>文章数\n");
  ...
  if (ch != 'A' && ch != 'T' && ch != 'W' && ch != 'C')
    {
      if (start > 1)
	printf
	  ("<th class=foot><a href=\"%s/bbstop?board=%s&go=U&to=%d%s\">上一页</a>\n",
	   BBSCGI, board, start, cookie_string);
      if (end < total)
	printf
	  ("<th class=foot><a href=\"%s/bbstop?board=%s&go=D&to=%d%s\">下一页</a>\n",
