...
print_table (char *information)
{
  ...
  printf ("<th class=foot><a href=\"%s/bbsreg\">新用户注册</a></th>", BBSCGI);
  printf ("<th class=foot><a href=\"%s/bbsman?id=guest\">游客参观</a></th>",
	  BBSCGI);
  printf ("</table>\n");
  printf ("</form>\n");
  printf ("</center></body>\n</html>\n");
...
main ()
{
  ...
  if (status == 1)
    print_table ("用户名或密码错误! 请重新输入");
  else if (status == 3)
    print_table ("您尚未登录! 请登录后再继续");
  else if (status == 4)
    print_table ("超时退出! 请重新登录");
