...
quote_string (char *buf)
{
  if (strstr (buf, "的大作中提到: 】") != NULL
      || strstr (buf, "□ 引用发信人:") != NULL
      || strstr (buf, "==>[作者]:") != NULL
      || (strstr (buf, "==>") != NULL && strstr (buf, ") 提到:") != NULL)
      || (strstr (buf, "【 在") != NULL
	  && strstr (buf, "的来信中提到: 】") != NULL)
...
print_go_bbsman ()
{
  printf ("<th class=foot><a href=\"%s/bbsman?%s\">主选单</a></th>",
...
print_go_bbssec ()
{
  printf ("<th class=foot><a href=\"%s/bbssec?%s\">分类讨论区</a></th>",
	  BBSCGI, cookie_string);
}
...
print_go_bbsall ()
{
  printf ("<th class=foot><a href=\"%s/bbsall?%s\">全部讨论区</a></th>",
	  BBSCGI, cookie_string);
}
...
print_go_bbs0an ()
{
  printf ("<th class=foot><a href=\"%s/bbs0an?%s\">精华公布栏</a></th>",
	  BBSCGI, cookie_string);
}
...
print_go_bbsmil ()
{
  if (strcmp (cookie.id, "guest") != 0)
    printf ("<th class=foot><a href=\"%s/bbsmil?%s\">处理邮件</a></th>",
	    BBSCGI, cookie_string);
}
...
{
  if (strcmp (cookie.id, "guest") == 0)
    printf ("<th class=foot><a href=\"%s/bbslog\">登录</a></th>", BBSCGI);
  else
