  ...
  while (fgets (buf, STRLEN, inf))
	...
	*ptr = '\0';
      if (strncmp (buf, "# Title=", 8) == 0)
	{
	  ...
	  printf ("<th class=title align=left width=33%%>精华区</th>\n");
	  ...
	  if (path[0] != '\0')
	    ...
	    printf
	      ("<th class=title align=right width=34%%>精华公布栏</th>\n");
	  printf ("</table>\n");
	  ...
	  if (num == 0)
	  ...
	  else
	    {
	      ...
	      printf ("<tr>");
	      printf
		("<th class=body>编号<th class=body>类别<th class=body>");
	      printf ("标题<th class=body>整理<th class=body>编辑日期\n");
	    }
	}
      ...
      else if (strncmp (buf, "Path=", 5) == 0)
	{
	  ...
	  if (!strstr (title, "(BM: BMS)") && !strstr (title, "(BM: SYSOPS)"))
		...
		{
		  printf ("错误<td class=body%d>", (index - 1) % 2 + 1);
		  print_title (title);
	...
	if (path[index] == '/')
	  {
	    path[index] = '\0';
	    printf
	      ("<th class=foot><a href=\"%s/bbs0an?path=%s%s\">上层目录</a>",
