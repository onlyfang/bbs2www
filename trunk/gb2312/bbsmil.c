...
main ()
{
  ...
  if (total == 0 && (ch == 'N' || ch == 'B'))
	...
	default:
      ...
      printf
	("<tr><th class=body>序号<th class=body>状态<th class=body>发信者");
  ...
  if (ch != 'A')
    {
      if (start > 1)
	printf
	  ("<th class=foot><a href=\"%s/bbsmil?go=U&to=%d%s\">上一页</a>\n",
	   BBSCGI, start, cookie_string);
      if (end < total)
	printf
	  ("<th class=foot><a href=\"%s/bbsmil?go=D&to=%d%s\">下一页</a>\n",
  ...
  printf
    ("<th class=foot><a href=%s/bbspsm?%s>发邮件</a>\n",
     BBSCGI, cookie_string);
