...
main ()
{
  ...
  if (total == 0 && (ch == 'N' || ch == 'B'))
	...
	default:
      ...
      printf
	("<tr><th class=body>序號<th class=body>狀態<th class=body>作者");
  ...
  if (ch == 'T' || ch == 'W' || ch == 'C')
    printf ("<th class=foot><a href=\"%s/bbsdoc?board=%s%s\">本討論區</a>\n",
  ...
  printf ("<th class=foot><a href=%s/bbstop?board=%s%s>主題模式</a>\n",
	  BBSCGI, board, cookie_string);
  ...
  if (ch != 'A' && ch != 'T' && ch != 'W' && ch != 'C')
    {
      if (start > 1)
	printf
	  ("<th class=foot><a href=\"%s/bbsdoc?board=%s&go=U&to=%d%s\">上一頁</a>\n",
	   BBSCGI, board, start, cookie_string);
      if (end < total)
	printf
	  ("<th class=foot><a href=\"%s/bbsdoc?board=%s&go=D&to=%d%s\">下一頁</a>\n",
