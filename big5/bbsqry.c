...
main ()
{
  ...
  login_check ();
  cgi_html_head ();
  printf ("<title>%s查詢网友</title>\n", BBSID);
  printf ("<body>\n");
  printf ("<center>\n");

  printf ("<table class=title width=90%%><tr>");
  printf ("<th class=title align=center>%s -- 查詢网友</th>\n", BBSNAME);
  printf ("</table>\n");
  printf ("<hr>\n");
  ...
  if (found == 0)
    {
      ...
      if (username[0] != '\0')
	printf ("<p>用戶名 <B>%s</B> 不存在!", username);
      else
	printf ("<p>請輸入用戶名:\n");

      printf ("<form action=\"%s/bbsqry\">", BBSCGI);
      printf ("<input type=submit value=\"查詢用戶\">");
  ...
  else
    {
      ...
      printf
	("<font class=col37>%s</font> (<font class=col33>%s</font>) 共上站 <font class=col32>%u</font> 次，發表過 <font class=col32>%u</font> 篇文章\n",
	 record.userid, record.username, record.numlogins, record.numposts);
      strcpy (buf1, ctime (&(record.lastlogin)));
      if ((newline = strchr (buf1, '\n')) != NULL)
	*newline = '\0';
      printf
	("上次在 [<font class=col32>%s</font>] 從 [<font class=col32>%s</font>] 到本站一游。\n",
	 buf1, (record.lasthost[0] == '\0' ? "(不詳)" : record.lasthost));
      ...
      printf
	("信箱：[<blink><font class=col32>%2s</font></blink>]，經驗值：[<font class=col32>%d</font>](<font class=col33>%s</font>) 表現值：[<font class=col32>%d</font>](<font class=col33>%s</font>) 生命力：[<font class=col32>%d</font>]。\n",
      ...
      for (i = 0; i < USHM_SIZE; i++)
	{
	  ...
	  if (strcmp (uin->userid, record.userid) == 0)
		...
		continue;
	      num++;
	      if (num == 1)
		printf ("<tr><td class=doc><pre>目前在站上，狀態如下：\n");
	...
	{
	  printf
	    ("<tr><td class=doc><font class=col36>沒有個人說明檔</font>\n");
	}
      else
	{
	  printf
	    ("<tr><td class=doc><font class=col36>個人說明檔如下：</font>\n");
