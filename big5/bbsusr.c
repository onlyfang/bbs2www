...
main ()
{
  ...
  printf ("<th class=title align=center>%s -- 在線用戶列表</th>\n", BBSNAME);
  printf ("</table>\n");
  ...
  if (num == 0)
    {
      printf ("<p>目前沒有用戶在站內</p>\n");
    }
  else
    {
      printf ("<table class=body>\n");
      printf ("<tr><th class=body>序號<th class=body>使用者代號");
      printf ("<th class=body>使用者昵稱<th class=body>來自<th class=body>");
      printf ("動態<th class=body>時:分\n");
